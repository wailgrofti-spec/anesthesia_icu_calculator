...
const double _kChipsH       = 42.0;
const String? _kAll         = null;

// ── Icônes des catégories (chips) ───────────────────────────────────────
// Association label → icône, résolue par correspondance partielle
// (insensible à la casse) afin de rester compatible avec les classes
// thérapeutiques générées dynamiquement depuis la base de données.
IconData _categoryIcon(String label) {
  final l = label.toLowerCase();
  if (l == 'tous') return Icons.grid_view_rounded;
  if (l.contains('hypno')) return Icons.bedtime_rounded;
  if (l.contains('opio')) return Icons.healing_rounded;
  if (l.contains('curare')) return Icons.vaccines_rounded;
  if (l.contains('vasopresseur')) return Icons.monitor_heart_rounded;
  if (l.contains('antibiotique')) return Icons.shield_rounded;
  if (l.contains('remplissage')) return Icons.water_drop_rounded;
  if (l.contains('sérum') || l.contains('serum') || l.contains('immunoglobuline')) {
    return Icons.opacity_rounded;
  }
  if (l.contains('autre')) return Icons.more_horiz_rounded;
...
class _DrugsScreenState extends State<DrugsScreen> {
  ...
  Map<String, List<Drug>> _groupByClass(List<Drug> list) {
    ...
    // Tri alphabétique (A→Z) des médicaments à l'intérieur de chaque classe.
        ...
        child: Column(children: [
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                // ── Ligne titre : badge + titre/sous-titre + recherche + bouton ──
                Padding(
                  padding: const EdgeInsets.fromLTRB(16, 14, 16, 10),
                  child: Row(
                    crossAxisAlignment: CrossAxisAlignment.center,
                    children: [
                      // Badge icône (capsule dans un carré arrondi bleu très clair)
                      Container(
                        width: 44, height: 44,
                        decoration: BoxDecoration(
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            ...
                            Text('Explorez et consultez les médicaments disponibles.',
                              maxLines: 1, overflow: TextOverflow.ellipsis,
                              style: TextStyle(
                                fontSize: 11,
                ...
                SizedBox(
                  ...
                  child: ListView(
                    ...
                    children: [
                      _CategoryChip(label: 'Tous', icon: _categoryIcon('Tous'),
                        active: _activeClass == null,
                      ...
                      ...DrugDatabase.therapeuticClassesInDataOrder
                        ...
                        .map((cls) => _CategoryChip(
                          label: cls, icon: _categoryIcon(cls),
                          active: _activeClass == cls,
                          isDark: _isDark,
                      ...
                      if (_autreSubClassesPresent.isNotEmpty)
                        _CategoryChip(
                          label: kAutreLabel, icon: _categoryIcon(kAutreLabel),
  ...
  Widget _buildDrugList(
    ...
    // L'utilisateur est entré dans une sous-catégorie de "Autre" (ex :
    // Insulines) depuis l'index "Autre" : on affiche une barre de retour
    // qui ramène directement à cet index plutôt qu'à "Tous".
    final cameFromAutre =
        _activeClass != null && kAutreTherapeuticClasses.contains(_activeClass);
...
// ════════════════════════════════════════════════════════════════════════════
//  BARRE RETOUR — vers l'index de la méta-catégorie "Autre"
// ════════════════════════════════════════════════════════════════════════════
class _BackToAutreBar extends StatelessWidget {
  final bool isDark;
  final VoidCallback onTap;
  const _BackToAutreBar({required this.isDark, required this.onTap});

  @override
  Widget build(BuildContext context) {
    final fg = isDark ? _DC.darkText : _DC.textPrimary;
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            border: Border(
              bottom: BorderSide(
                color: isDark ? _DC.darkBorder : _DC.divider, width: 1))),
          child: Row(children: [
            Icon(Icons.arrow_back_rounded, size: 17, color: _DC.accent),
            const SizedBox(width: 8),
            Text(kAutreLabel,
              style: TextStyle(
...
class _CategoryChip extends StatelessWidget {
  ...
  final VoidCallback onTap;
  const _CategoryChip({required this.label, required this.icon, required this.active,
  ...
  Widget build(BuildContext context) {
    final bg = active
      ? _DC.accent
      : (isDark ? _DC.darkCard : Colors.white);
    final fg = active ? Colors.white : (isDark ? _DC.darkSub : _DC.textSecond);
    final border = active
      ? _DC.accent
      : (isDark ? _DC.darkBorder : const Color(0xFFE2E8F0));
    return Padding(
      ...
      child: GestureDetector(
        ...
        child: AnimatedContainer(
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: active
              ? [BoxShadow(
                  color: _DC.accent.withOpacity(0.25),
...
class _CompactSearchBarState extends State<_CompactSearchBar> {
  ...
  Widget build(BuildContext context) {
    ...
    final fieldBg = isDark ? _DC.darkInput : Colors.white;
    final txtP    = isDark ? _DC.darkText  : _DC.textPrimary;
    final txtHint = isDark ? _DC.darkSub   : _DC.textMuted;
      ...
      builder: (context, _) {
        ...
        return AnimatedContainer(
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(
              color: _focused
                  ? _DC.accent.withOpacity(0.35)
                  : (isDark ? _DC.darkBorder : const Color(0xFFE6EAF2)),
              width: _focused ? 1.4 : 1,
            ),
            boxShadow: _focused
                ? [BoxShadow(
                    color: _DC.accent.withOpacity(0.08),
                    blurRadius: 0,
                    spreadRadius: 4,
                  )]
                : [],
          ...
          child: Row(children: [
            ...
            Icon(Icons.search_rounded, size: 19,
                color: _focused || hasText ? _DC.accent
                    : (isDark ? _DC.darkSub : _DC.textMuted)),
            const SizedBox(width: 8),
            Expanded(
              child: TextField(
            ...
            if (hasText)
              GestureDetector(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    color: isDark ? _DC.darkBorder : const Color(0xFFF1F3F7),
                    shape: BoxShape.circle,
                  ),
